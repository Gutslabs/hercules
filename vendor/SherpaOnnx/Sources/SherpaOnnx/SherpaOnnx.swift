...
import SherpaGuard   // C++ exception guard — onnxruntime fırlatınca çökmek yerine nil
...
class SherpaOnnxOfflineTtsWrapper {
  ...
  init(
    ...
    // Doğrudan SherpaOnnxCreateOfflineTts yerine C++ guard: onnxruntime oturum
    // açarken exception fırlatırsa guard NULL döner (çökmez) → tts = nil →
    // SherpaTTSEngine.init? nil → uygulama Apple sistem sesine düşer.
    let raw = SherpaGuardCreateOfflineTts(UnsafeRawPointer(config))
    tts = raw.map(OpaquePointer.init)
  }
