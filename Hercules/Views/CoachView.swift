...
struct CoachView: View {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: Spacing.md) {
            ...
            if let errorText {
                HStack(spacing: 8) {
                    Lucide(sf: "exclamationmark.triangle.fill").foregroundStyle(Palette.warning)
    ...
    private func railFocusRow(_ item: CoachFocusItem) -> some View {
        HStack(alignment: .top, spacing: 8) {
            Lucide(sf: statusIcon(item.status), size: 11).foregroundStyle(statusTint(item.status)).frame(width: 16)
        ...
        VStack(alignment: .leading, spacing: Spacing.lg) {
            ...
            HStack {
                ...
                Button { saveRecipeToLibrary(recipe) } label: {
                    HStack(spacing: 7) {
                        Lucide(sf: saved ? "checkmark" : "bookmark", size: 11)
    ...
    private func sourceRow(_ recipe: CoachRecipe) -> some View {
        if let urlString = recipe.sourceURL, let url = URL(string: urlString) {
            Button { openURL(url) } label: {
                HStack(spacing: 8) {
                    Lucide(sf: "link", size: 11).foregroundStyle(Palette.accent)
    ...
    private func emptyState(recipe: Bool) -> some View {
        VStack(alignment: .leading, spacing: Spacing.md) {
            ZStack {
                ...
                Lucide(sf: recipe ? "fork.knife" : "sparkles", size: 24).foregroundStyle(Palette.accent)
            }
