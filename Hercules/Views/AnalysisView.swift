...
struct AnalysisView: View {
    ...
    private func rateBand(rate: Double) -> some View {
        ...
        return ZoneBar(
            zones: band.zones.map {
                ZoneBar.Zone(width: $0.width, color: $0.color, label: $0.label, highlighted: $0.ideal)
            },
    ...
    private func karneRow(_ item: ScienceEngine.ScoreItem) -> some View {
        HStack(spacing: 14) {
            ...
            GoalBar(
                value: item.progress,
                goal: 1,
                tint: toneColor(item.tone).opacity(0.8),
                height: 4,
                hatched: false,
                headMarker: false,
                overflowTint: toneColor(item.tone).opacity(0.8),
    ...
    private func volumeRow(_ mv: ScienceEngine.MuscleVolume) -> some View {
        ...
        return HStack(spacing: 14) {
            ...
            // Ray = 0…MRV+5 seti; yeşil şerit MEV–MRV verimli bandı, iki çizgi eşikler.
            GoalBar(
                value: mv.sets,
                goal: scale,
                ...
                headMarker: false,
                markers: [
                    BarMarker(ratio: l.mev / scale, color: Palette.textTertiary, cap: false, extends: 4),
                    BarMarker(ratio: l.mrv / scale, color: Palette.textTertiary, cap: false, extends: 4),
                ],
                band: BarBand(from: l.mev / scale, to: l.mrv / scale, color: Palette.positive.opacity(0.16)),
                overflowTint: color.opacity(0.85),
                trackColor: Palette.track
            )
    ...
    private func liftRow(_ lift: ScienceEngine.LiftProgress) -> some View {
        HStack(spacing: 12) {
            ...
            HStack(spacing: 3) {
                Lucide(sf: trendIcon(lift.trend), size: 8)
...
// e1RM satırlarındaki mini grafik `TrendSpark`'a taşındı (Theme/MetricComponents.swift).
