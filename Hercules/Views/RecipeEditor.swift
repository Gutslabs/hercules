...
struct RecipeEditor: View {
    ...
    var body: some View {
        SheetChrome(
            eyebrow: isEditing ? "Tarifi Düzenle" : "Yeni Tarif",
            title: trimmedTitle.isEmpty ? "Tarif taslağı" : trimmedTitle,
            icon: category.icon,
            iconTint: category.displayTint,
            size: .full,
            onClose: { dismiss() }
        ) {
        ...
        VStack(alignment: .leading, spacing: Spacing.lg) {
            VStack(alignment: .leading, spacing: Spacing.md) {
                HStack(spacing: Spacing.sm) {
                    Lucide(sf: category.icon, size: 12)
                    Text(category.label)
                        .font(Typography.captionBold)
                    Spacer()
                    if isFavorite {
                        Lucide(sf: "heart.fill", size: 11)
            ...
            VStack(alignment: .leading, spacing: 9) {
                ...
                ForEach(Array(completionRows.enumerated()), id: \.offset) { _, row in
                    HStack(spacing: Spacing.sm) {
                        Lucide(sf: row.1 ? "checkmark.circle.fill" : "circle", size: 11)
        ...
        VStack(alignment: .leading, spacing: Spacing.lg) {
            editorSection("Kimlik", icon: "tag") {
                VStack(alignment: .leading, spacing: Spacing.md) {
                    ...
                    Toggle(isOn: $isFavorite) {
                        HStack(spacing: Spacing.sm) {
                            Lucide(sf: isFavorite ? "heart.fill" : "heart", size: 12)
            ...
            if isEditing, let onDelete {
                Button(role: .destructive) {
                    ...
                    Label { Text("Tarifi Sil") } icon: { Lucide(sf: "trash") }
                        .font(Typography.captionBold)
                        .frame(maxWidth: .infinity)
                        .padding(.vertical, 11)
                }
