...
struct DashboardView: View {
    ...
    var body: some View {
        ...
        // `consumedCalories` gözlemi KALDIRILDI: öğün eklenince hem sayı hem toplam
        // değişiyor ve üç senkron tam-tablo fetch'i iki kez çalışıyordu. Üstelik
        // onChange değerini her body geçişinde hesapladığı için toplam O(n) reduce'u
        // da bedavaya gidiyordu.
    ...
    private func latestStepsByDay(_ steps: [StepEntry], calendar: Calendar) -> [Date: StepEntry] {
        StepEntry.preferredEntries(from: steps, calendar: calendar).reduce(into: [:]) { result, entry in
