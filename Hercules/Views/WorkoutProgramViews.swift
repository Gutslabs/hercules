        ...
        VStack(spacing: 0) {
            ...
            HStack(spacing: 12) {
                ...
                Button {
                    ...
                    Lucide(sf: isOpen ? "chevron.up" : "chevron.down", size: 9)
                        .foregroundStyle(Palette.textTertiary)
...
struct WorkoutProgramArchiveSheet: View {
    ...
    var body: some View {
        // Footer'sız görüntüleme sheet'i: kabuk ayracı ve boş alt şeridi kendiliğinden atlıyor.
        SheetChrome(
            eyebrow: "Antrenman",
            title: "Program Arşivi",
            subtitle: archives.isEmpty ? "Arşivlenmiş program yok." : "\(archives.count) arşiv",
            size: .standard,
            onClose: { dismiss() }
        ) {
                ...
                ForEach(archives) { archive in
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        HStack(spacing: Spacing.sm) {
                            SheetPrimaryButton("Geri Yükle", icon: "arrow.clockwise") {
                                onRestore(archive)
                                dismiss()
                            }
                            SheetDestructiveButton("Sil") { onDelete(archive) }
                            Spacer(minLength: 0)
                        }
                        .padding(.top, 2)
                    }
