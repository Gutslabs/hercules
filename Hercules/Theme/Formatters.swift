...
/// Sürekli "nabız/nefes" animasyonlarını TimelineView'a bağlamak için saat yardımcısı.
/// `withAnimation(.repeatForever)` `.onAppear`'da SwiftUI'de LEAK ediyor: view ağaçtan
/// çıkınca (sayfa değişince) render döngüsü refresh-rate'te dönmeye devam ediyor → tüm
/// pencere boştayken bile her karede yeniden çiziliyor (görünmez ama AÇIK olan her sayfayı
/// kasar — Takvim'in "boştayken bile kasması"nın kök nedeni buydu). `TimelineView(.animation)`
/// ise view görünmez olunca DURUR → leak yok. (Theme'de: hem Mac hem iOS target'ında erişilir.)
enum Pulse {
    /// lo…hi arası yumuşak (easeInOut benzeri) gidip-gelen nefes; tam tur `period` sn.
    static func breathe(_ date: Date, period: Double, _ lo: Double, _ hi: Double) -> Double {
        let t = date.timeIntervalSinceReferenceDate.truncatingRemainder(dividingBy: period)
        let s = (1 - cos(2 * .pi * t / period)) / 2     // 0→1→0
        return lo + (hi - lo) * s
    }
    /// 0→1 easeOut rampası, her `period` sn'de tekrarlar (halka/ping efekti).
