...
final class WorkoutLog {
    ...
    var estimatedCalories: Double = 300
    /// Kaydın son değişiklik zamanı.
    var updatedAt: Date = Date.now
    /// CloudKit: to-many optional olmalı. Stored optional + originalName (eski veri korunur),
    /// dışa API non-optional computed accessor.
    @Relationship(deleteRule: .cascade, originalName: "exercises", inverse: \WorkoutExerciseEntry.log)
    var exercisesStorage: [WorkoutExerciseEntry]? = []
...
final class WorkoutExerciseEntry {
    ...
    var order: Int = 0
    /// CloudKit inverse: ait olduğu seans logu.
    var log: WorkoutLog?
    /// CloudKit: to-many optional olmalı. Stored optional + originalName, dışa non-optional computed.
    @Relationship(deleteRule: .cascade, originalName: "setEntries", inverse: \ExerciseSet.entry)
    var setEntriesStorage: [ExerciseSet]? = []

    var setEntries: [ExerciseSet] {
        get { setEntriesStorage ?? [] }
        set { setEntriesStorage = newValue }
    }
...
final class ExerciseSet {
    ...
    var weight: Double?
    /// CloudKit inverse: ait olduğu hareket.
