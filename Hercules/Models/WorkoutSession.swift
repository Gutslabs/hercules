...
final class WorkoutPlanOverride {
    ...
    var weekday: Int = 1
    var operationRaw: String = "add_exercise"
...
final class WorkoutTemplateExercise {
    var name: String = ""
    var order: Int = 0
    /// CloudKit inverse: ait olduğu haftalık program günü.
...
final class WorkoutSession {
    ...
    /// CloudKit to-many'nin OPTIONAL olmasını şart koşar → stored optional. `originalName`
    /// eski store'daki "templateExercises" ilişkisini bu alana taşır (veri korunur). Dışa
    /// dönük API `templateExercises` non-optional kalsın diye computed accessor sarar.
    @Relationship(deleteRule: .cascade, originalName: "templateExercises", inverse: \WorkoutTemplateExercise.session)
    var templateExercisesStorage: [WorkoutTemplateExercise]? = []

    var templateExercises: [WorkoutTemplateExercise] {
        get { templateExercisesStorage ?? [] }
        set { templateExercisesStorage = newValue }
    }
