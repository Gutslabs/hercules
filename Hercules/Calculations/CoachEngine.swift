...
enum CoachEngine {
    ...
    enum CoachError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .unverifiedRecipeSource:
                return "Tarif, tamamlanmış web aramasındaki gerçek bir kaynak URL'siyle doğrulanamadı; kaydedilmedi."
            }
        }
    }
    ...
    // Bu üç arama tek satır istiyor ama eskiden tüm tabloyu (günde bir satır büyüyen,
    // yani sonsuza dek büyüyen) çekip bellekte filtreliyordu — üstelik açılışta,
    // foreground'a dönüşte ve CoachView.onAppear'da. Artık predicate + fetchLimit.
    ...
    static func existingReport(for day: Date, ctx: ModelContext) -> CoachReport? {
        ...
        var descriptor = FetchDescriptor<CoachReport>(
    ...
    static func existingRecipe(for day: Date, ctx: ModelContext) -> CoachRecipe? {
        ...
        let start = calendar.startOfDay(for: day)
        let end = calendar.date(byAdding: .day, value: 1, to: start) ?? day
        var descriptor = FetchDescriptor<CoachRecipe>(
            predicate: #Predicate { $0.day >= start && $0.day < end },
            sortBy: [SortDescriptor(\.day, order: .reverse)]
        )
        descriptor.fetchLimit = 1
    ...
    static func generateDailyRecipe(ctx: ModelContext) async throws -> CoachRecipe {
        ...
        let (result, searchEvidence) = try await AIKeyStore.shared.makeClient().send(
        ...
        guard searchEvidence?.completedSuccessfully == true,
              let action,
              action.sourceVerified,
              action.unambiguousRecipeSourceURL.flatMap(
                  AIWebSearchEvidence.canonicalURL
              ) == action.verifiedSourceCanonicalURL,
              searchEvidence?.contains(action.unambiguousRecipeSourceURL) == true
        else {
            throw CoachError.unverifiedRecipeSource
        }
        ...
        let sourceURL = action.unambiguousRecipeSourceURL?
...
enum CoachLaunchAgent {
    ...
    static func installIfNeeded() {
        ...
        do {
            ...
            // Atomik: launchd bu dizini izliyor, yarım yazılmış bir plist'i okursa
            // ajan hiç yüklenmez. Kodun geri kalanındaki tüm yazımlar da atomik.
