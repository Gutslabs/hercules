...
final class ResearchLibrary {
    ...
    func updateNow(
        ...
        // böylece research-library.json sınırsız büyümez.
    ...
    /// Diskteki dosyanın son okunan hâli (boyut + değişim tarihi). Değişmediyse
    /// yeniden okuma+decode yapılmaz — `allPapers()` her pin/sil/yenile işleminde
    /// çağrıldığı için tüm kütüphane ana thread'de tekrar tekrar decode ediliyordu.
    /// (Aynı korumanın hafıza tarafındaki karşılığı: refreshFromDiskIfChanged.)
    private struct DiskStamp: Equatable {
        var size: Int
        var modified: Date
    }
    private var loadedStamp: DiskStamp?

    private func currentDiskStamp() -> DiskStamp? {
        guard let values = try? researchURL.resourceValues(forKeys: [.fileSizeKey, .contentModificationDateKey]),
              let size = values.fileSize,
              let modified = values.contentModificationDate
        else { return nil }
        return DiskStamp(size: size, modified: modified)
    }

    private func load() {
        let stamp = currentDiskStamp()
        if let stamp, let loadedStamp, stamp == loadedStamp { return }
    ...
    private func persist() {
        ...
        // RAM zaten güncel; damgayı da tazele ki sonraki load() boşuna okumasın.
        loadedStamp = currentDiskStamp()
    }
