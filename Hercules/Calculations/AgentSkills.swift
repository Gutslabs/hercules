...
struct MemoryRecallSkill: AgentSkill {
    ...
    func run(query: String, context: AgentContext) async throws -> SkillResult? {
        let retrievalQuery = Self.retrievalQuery(query, history: context.history)
        // Model yüklüyse semantic (cosine) skoru da harmanlanır; değilse saf lexical (graceful).
        let queryEmbedding = await EmbeddingService.shared.embedQueryIfAvailable(retrievalQuery)
        // LocalMemoryProvider artık @MainActor — erişimi main'e taşı (veri yarışını önler).
        let content = await MainActor.run { () -> String? in
            let memories = LocalMemoryProvider.modelSafeMemories(
                LocalMemoryProvider.shared.contextMemories(
            ...
            // Geniş depo özeti yalnız kullanıcı gerçekten "beni ne kadar tanıyorsun /
            // hafızanda ne var" diye sorarsa eklenir. Her turda ilgili facts + bütün
            // digest'i birlikte vermek context'i seyreltir ve token maliyetini şişirir.
            if Self.isBroadMemoryRequest(query),
               let digest = MemoryDigest.shared.digestBlock() {
                lines.append("")
                lines.append(digest)
            }
    ...
    private static func retrievalQuery(_ query: String, history: [ChatTurn]) -> String {
        let normalized = AgentQueryClassifier.normalized(query)
        let deictic = AgentQueryClassifier.containsAny(normalized, [
            "bu", "bunu", "buna", "bundan", "o", "onu", "ona", "dediğim",
            "demiştim", "önceki", "aynı", "devam", "peki"
        ])
        guard query.count < 80, deictic else { return String(query.prefix(1_000)) }
        let recentUser = history
            .filter { $0.role == .user }
            .suffix(2)
            .map(\.text)
            .joined(separator: "\n")
        guard !recentUser.isEmpty else { return String(query.prefix(1_000)) }
        return String((recentUser + "\n" + query).suffix(1_000))
    }

    private static func isBroadMemoryRequest(_ query: String) -> Bool {
        let normalized = AgentQueryClassifier.normalized(query)
        return AgentQueryClassifier.containsAny(normalized, [
            "hafizanda ne var", "hafizamda ne var", "beni ne kadar taniyorsun",
            "hakkimda ne biliyorsun", "tum hafiza", "hafiza ozeti", "memory ozeti",
            "beni nasil hatirliyorsun"
        ])
    }
}
