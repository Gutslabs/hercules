...
struct AgentDataScope: Sendable {
    ...
    static func infer(
        ...
        let nutrition = tags.contains(.kalori) || tags.contains(.takvim) || tags.contains(.yemekPlani) || containsAny(lower, AgentQueryClassifier.nutritionSignals)
        ...
        let recipe = tags.contains(.tarifler) || tags.contains(.yemekPlani) || containsAny(lower, [
        ...
        if containsAny(lower, ["adim", "step", "hareket", "yuru", "neat", "cut", "definasyon"]) || tags.contains(.genelBakis) {
...
struct AgentDataSnapshot: Sendable {
    ...
    private static func fetchSteps(
        ...
        let preferred = StepEntry.preferredEntries(
            from: (try? ctx.fetch(descriptor)) ?? []
        ).sorted { $0.date > $1.date }
        if let limit { return Array(preferred.prefix(limit)) }
        return preferred
    }
